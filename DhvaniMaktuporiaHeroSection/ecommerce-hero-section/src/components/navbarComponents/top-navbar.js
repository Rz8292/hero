...
function TopNavbar({topWidgetVisibility}){
    ...
    const fetchLanguages = async () => {
        try {
            ...
            if (response.ok) {
                ...
                const languagesArray = Array.from(languagesSet).map((lang, index) => ({
                    name: lang,
                    code: `lang-${index}`,
                }));
                setLanguages(languagesArray); // RestCountries API returns languages under 'languages' object
            } else {
    ...
    const fetchCurrencies = async () => {
        try {
            ...
            if (response.ok) {
                const data = await response.json();
                const currenciesArray = Object.keys(data.rates).map((currency) => ({
                    name: currency,
                    symbol: currency,
                }));
                setCurrencies(currenciesArray); // ExchangeRate-API returns currencies as keys in 'rates'
            } else {
    ...
    return(
        <>
            <div className='top-navbar'>
                    ...
                    {/* social media icons */}
                    <p className='follow-p' style={{marginLeft: "10%"}}>Follow us:</p>
